...
function createBinflow(stru, endian) {
  ...
  function _parseArrayToken(buf, offset, token) {
    const type = token[0];
    const count = token[1];
    const itemSize = _getValueSize(type);
    const size = itemSize * count;
    ...
    if (type === 'byte') {
      ...
      value = Array.from(new Array(count)).map((v, idx) => {
        const itemOffset = offset + (itemSize * idx);
        const val = bnf.parse(buf, itemOffset);
        return val;
      });
    } else {
      value = Array.from(new Array(count)).map((v, idx) => {
        const itemOffset = offset + (itemSize * idx);
        const val = _readByType(buf, itemOffset, type, _struEndian);
        return val;
      });
    }
  ...
  function encode(values, baseBuf) {
    let offset = 0;
    const bufSize = getStructureSize(_structure);
  ...
  function _encodeArrayToken(value, buf, offset, token) {
    ...
    if (type === 'byte') {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const itemOffset = offset + (itemSize * i);
        const bufVal = bnf.encode(val, buf.slice(itemOffset));
        bufVal.copy(buf, itemOffset);
      }
    } else {
      for (let i = 0; i < count; i++) {
        const val = value[i];
        const itemOffset = offset + (itemSize * i);
        _writeByType(val, buf, itemOffset, type, _struEndian);
      }
    }
 ...
 * @param   {object} stru binflow structure
 * @returns {number} the size of structure
 * @example
 * const binflow = require('binflow');
 * const stru = {
 *   prop1: 'int8',
 ...
 */
function getStructureSize(stru) {
 ...
 * @param   {object} stru binflow structure
 * @param   {string} field the field to get the size of
 * @param   {string} [subfield] the subfield of the field
 * @returns {number} the size of field
 * const binflow = require('binflow');
 * const stru = {
 *   prop1: 'int8',
 *   prop2: ['uint16', 2],
 ...
 */
function getFieldSize(stru, field, subfield) {
  const { token } = _getTokenOffsetOfField(stru, field, subfield);
  return _getValueSize(token);
}

function _hasEndian(type) {
  const tail = type.toString().slice(-2);
...
function _getValueSize(token) {
  if (Array.isArray(token)) {
    const itemType = token[0];
    const count = token[1];
    const itemSize = _getValueSize(itemType);
    return itemSize * count;
  } else if (typeof token === 'object') {
    return getStructureSize(token);
  }
...
function _isIntType(type) {
  return type.toString().indexOf('int') === 0;
}

function _isUIntType(type) {
  return type.toString().indexOf('uint') === 0;
}
